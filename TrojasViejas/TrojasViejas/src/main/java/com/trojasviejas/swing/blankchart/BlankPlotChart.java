package com.trojasviejas.swing.blankchart;
...
public class BlankPlotChart extends JComponent {
    ...
    public BlankPlotChartRender getBlankPlotChartRender() {
        return blankPlotChartRender;
    }

    public void setBlankPlotChartRender(BlankPlotChartRender blankPlotChartRender) {
        this.blankPlotChartRender = blankPlotChartRender;
    }
    ...
    private BlankPlotChartRender blankPlotChartRender;
    ...
    private void renderSeries(Graphics2D g2) {
        if (blankPlotChartRender != null) {
            ...
            double locationX = insets.left + textWidth + spaceText;
            for (int i = 0; i < labelCount; i++) {
                blankPlotChartRender.renderSeries(this, g2, getRectangle(i, height, space, locationX, insets.top), i);
            }
            List<Path2D.Double> gra = initGra(blankPlotChartRender.getMaxLegend());
            for (int i = 0; i < labelCount; i++) {
                blankPlotChartRender.renderSeries(this, g2, getRectangle(i, height, space, locationX, insets.top), i, gra);
            }
            blankPlotChartRender.renderGraphics(g2, gra);
        }
    }
    ...
    private void mouseMove(Graphics2D g2, MouseEvent evt) {
        if (blankPlotChartRender != null) {
            ...
            double locationX = insets.left + textWidth + spaceText;
            for (int i = 0; i < labelCount; i++) {
                boolean stop = blankPlotChartRender.mouseMoving(this, evt, g2, getRectangle(i, height, space, locationX, insets.top), i);
                if (stop) {
                    break;
                }
            }
    ...
    private String getChartText(int index) {
        if (blankPlotChartRender != null) {
            return blankPlotChartRender.getLabelText(index);
        } else {
            return "Label";
        }
    }
